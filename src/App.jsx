...
const FinancialReports = ({ policies, paymentsExpenses, loadingPolicies, loadingPaymentsExpenses, formatDate }) => {
    ...
    // Corrected Calculation: Commission only for policies NOT paid to insurer
...
const App = () => {
    ...
    // Policy Filters and Sort States for View Policies
    ...
    // Edit Modals State (shared across App, passed as props to relevant components)
    ...
    const [selectedPolicyForPayment, setSelectedPolicyForPayment] = useState('');
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const fetchedPaymentsExpenses = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    ...
    return (
        <div className="min-h-screen bg-[#F0F2F5] font-sans text-gray-800">
            ...
            <div className="flex flex-col lg:flex-row min-h-screen">
                ...
                {/* These modals are global and can remain here or be moved if desired */}
                {isPolicyEditModalOpen && (
                    <EditPolicyModal
                        policy={selectedPolicyForEdit}
                        onClose={() => setIsPolicyEditModalOpen(false)}
