...
// Import auth, db, and authentication functions from your Firebase.js
import { auth, db, onAuthStateChanged, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut } from './Firebase.js'; // <-- Added .js extension
...
const App = () => {
        ...
        const uniqueCustomers = policies.reduce((acc, policy) => {
            if (policy.customer && policy.customer.idNumber) {
                ...
                acc[policy.customer.idNumber].associatedPolicies.push({
                    id: policy.id, policyNumber: policy.policyNumber, policyType: policy.policyType,
                    totalAmount: policy.totalAmount, policyDate: policy.policyDate
                });
            }
            return acc;
        }, {});
                                    ...
                                    <tr key={customer.idNumber || index} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button
                                                ...
                                                className="text-indigo-600 hover:text-indigo-900 p-1 rounded-md bg-indigo-50 hover:bg-indigo-100 transition"
                                            >
                                                Edit
                                            </button>
                                        </td>
    ...
    const renderPage = useCallback(() => {
        if (!isAuthReady) {
            return (
                <div className="flex justify-center items-center h-full text-indigo-600 text-xl font-semibold">
                    <svg className="animate-spin -ml-1 mr-3 h-8 w-8 text-indigo-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                ...
                <div className="flex items-center justify-center min-h-[calc(100vh-120px)] p-4">
                    <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
                        ...
                        <form onSubmit={handleAuth} className="space-y-4">
                            ...
                            <button type="submit"
                                className="w-full bg-indigo-600 text-white p-3 rounded-md font-semibold hover:bg-indigo-700 transition duration-200">
                                {isLogin ? 'Login' : 'Register'}
                            </button>
                        </form>
                ...
                return (
                    <div className="p-6 bg-white rounded-xl shadow-sm space-y-6">
                        ...
                        <form onSubmit={handleAddPolicy} className="space-y-6">
                            ...
                            <button type="submit"
                                className="w-full bg-indigo-600 text-white p-3 rounded-md shadow-lg hover:bg-indigo-700 transition duration-300 ease-in-out transform hover:scale-105">
                                Add Policy
                            </button>
                        </form>
                    </div>
